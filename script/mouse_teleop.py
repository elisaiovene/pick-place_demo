...
class MouseTeleop():
    def __init__(self):
        ...
        self._pub_grip = rospy.Publisher('mouse_grip', Float32, queue_size=100)
        self._pub_scroll = rospy.Publisher('mouse_scroll', Float32, queue_size=100)
        self._pub_change = rospy.Publisher('mouse_change', Float32, queue_size=100)
        ...
        self._canvas.bind('<Button-2>', self._middle)
        self._canvas.bind('<Button-3>', self._grip)
        self._canvas.bind('<ButtonRelease-3>', self._grip_release)
    ...
    def _start(self, event):
        ...
        self._v_x = self._v_y = self._w = self._v_z = 0.0
    ...
    def _send_scroll(self):
        scroll = self.count
        data = scroll 
        self._pub_scroll.publish(data)

    def _send_change(self):
        change = self.change
        data = change
        self._pub_change.publish(data)
    
    def _send_grip(self):
        grip = self.butt
        data_grip = grip
        self._pub_grip.publish(data_grip)
